...
const CustomerForm = ({ showDlg, toggleDlg, title, selectedData, userId }: CustomerFormProp) => {
    ...
    // const userId = useRef(user.data.id)
    ...
    const submit: FormEventHandler = async (e) => {
        ...
        if (isEditMode) {
            // Update the client
            await put(`/api/client/${selectedData.client_id}`, {
                onSuccess: () => {
    ...
    return (
        <Dialog open={showDlg} onOpenChange={toggleDlg}>
            <DialogContent className="dark:bg-slate-800 xl:max-w-[48vw] xl:max-h-[50vh]">
